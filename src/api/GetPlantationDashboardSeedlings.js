...
import localPlantationDashboardSeedlings from '../locals/localPlantationDashboardSeedlings'
import useGetRequest from '../hooks/useGetRequest'

// Make an API by creating a function.
export default function GetPlantationDashboardSeedlings() {
  // Get environment variables.
  const { API_QLIFE, LOCAL_MODE } = useEnv()

  // Configure the HTTP request.
  const path = '/dashboard/totalSeedlings'
  const url = API_QLIFE + path

  // If local mode is true, return local data.
  if (LOCAL_MODE === true) {
    useDevLogs(`GET: ${url}`, null, localPlantationDashboardSeedlings)
    return { data: localPlantationDashboardSeedlings }
  }
